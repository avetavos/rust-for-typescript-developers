...
export const binCrateTree = `flowchart TD
...
export const targetTree = `flowchart TD
...
<Mermaid code={binCrateTree} title="What cargo new gives you (binary crate)" />
...
<Mermaid code={libCrateTree} title="Library crate layout" />
...
name = "my-app"
version = "0.1.0"
edition = "2024"     # Rust edition (like target in tsconfig)
...
<TsGo ts={cargoTomlTs} go={cargoTomlRs} />

ความแตกต่างหลัก:
- ไม่มี Section `scripts` คุณรัน `cargo run`, `cargo test` ฯลฯ โดยตรง
- `edition` สอดคล้องกับ Rust language edition (2015, 2018, 2021, 2024) ใช้ `2024` เสมอสำหรับโปรเจกต์ใหม่
...
<Mermaid code={moduleTree} title="A project with modules" />
...
<TsGo ts={modulesTs} go={modulesRs} />
...
<Mermaid code={targetTree} title="The target/ directory" />
...
Rust ใช้ **Editions** เพื่อนำ Breaking changes เข้ามาในภาษาโดยยังรักษา Backward compatibility ไว้ได้ Edition กำหนดแยกต่อโปรเจกต์ใน `Cargo.toml` และไม่ส่งผลต่อ ABI — Crate ที่อยู่คนละ Edition ทำงานร่วมกันได้อย่างอิสระ

Edition ปัจจุบัน: 2015, 2018, 2021, 2024 เริ่มโปรเจกต์ใหม่ด้วย `edition = "2024"` เสมอ — `cargo new` เลือกให้อยู่แล้ว

คล้ายกับ setting `target` และ `lib` ของ TypeScript ใน `tsconfig.json` แต่ง่ายกว่า: ตั้งค่าครั้งเดียวแล้ว Compiler จัดการที่เหลือให้หมด และต่างจากการ bump `tsconfig` ตรงที่การย้ายโปรเจกต์เก่าไป edition ใหม่แทบเป็นงานอัตโนมัติ — `cargo fix --edition` แก้ code ให้เอง
