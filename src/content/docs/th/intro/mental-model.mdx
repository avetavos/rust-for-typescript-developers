...
<TsGo ts={ownershipTs} go={ownershipRs} />
...
<TsGo ts={expressionsTs} go={expressionsRs} />
...
<Playground code={mentalModelCode} />

<RsOnly title="ตัวดำเนินการ ? — การส่งต่อ errors อย่างสะดวก">

การ match ทุก `Result` ทำให้โค้ดยืดยาวได้ Rust จึงมีตัวดำเนินการ `?` มาให้: วางไว้หลัง `Result` แล้วจะ unwrap ค่า `Ok` หรือไม่ก็ return `Err` กลับไปให้ caller ทันที เทียบได้กับการ re-throw exception แต่ปรากฏชัดอยู่ใน function signature
...
export const quizMentalModel = [
  ...
  },
  {
    ...
    options: [
      ...
      "หนึ่งตัวเท่านั้น และห้ามมี immutable reference อยู่ร่วมด้วย",
  ...
  },
  {
    ...
    options: [
      "เป็น ternary syntax พิเศษที่มีเฉพาะใน Rust",
