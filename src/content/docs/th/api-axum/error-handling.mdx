...
```rust
// @skip-verify crate: axum (Path extractor, fragment building on this lesson's AppError)
async fn get_user(Path(id): Path<u32>) -> Result<Json<User>, AppError> {
...
<TsGo ts={tsErrorUnion} go={rsThiserror} />
...
แปลว่า error ทุกแบบที่ handler สร้างได้ **มองเห็นได้จาก return type** ไม่มี implicit error path ไม่มี uncaught async rejection และไม่มีบั๊ก "ลืมเรียก `next(err)`" ตัว compiler จะเช็กให้ว่าทุก error variant ถูกจัดการครบ — ไม่ว่าจะปล่อยให้ `?` propagate ขึ้นไปที่ `Result` ของ handler หรือ map ไปเป็น variant อื่น
