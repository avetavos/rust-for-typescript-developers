...
- `edition` corresponds to the Rust language edition (2015, 2018, 2021, 2024). Always use `2024` for new projects.
...
Current editions: 2015, 2018, 2021, 2024. Always start new projects with `edition = "2024"` — `cargo new` already picks it for you.

This is analogous to TypeScript's `target` and `lib` settings in `tsconfig.json`, but simpler: you set it once and the compiler handles everything. Unlike a `tsconfig` bump, moving an old project forward is mostly mechanical — `cargo fix --edition` rewrites the code for you.
